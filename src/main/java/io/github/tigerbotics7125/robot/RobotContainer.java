...
public class RobotContainer {
	...
	mDrivetrain.setDefaultCommand(mDrivetrain.drive(mDriver.leftY()::get, mDriver.leftX()::get,
	...
	/**
	 * Givin a game like RAPID REACT, you should really be averaging a pose
	 * between all of the seen hub tags, so that you would not shoot over
	 * the side of the hub.
	 */

	// Set drivetrain target lock.
	if (kTargetLockTags.contains(bestTarget.getFiducialId())) {
	    Pose2d tagPose = mVision.getTagLayout().getTagPose(bestTarget.getFiducialId()).toPose2d();
	    Pose2d robotPose = mDrivetrain.getPose();

	    Transform2d robotToTag = new Transform2d(robotPose, tagPose);

	    // Law of sines used to find the rotation needed to face the target.
	    Rotation2d faceTarget = new Rotation2d(robotToTag.getY() * Math.sin(Math.PI / 2) / robotToTag.getX());
	...
	if (correcting) {
		...
		mDrivetrain.addVisionMeasurement(mVision.getRobotPoseFromTarget(target).toPose2d(),
	...
	if (Robot.isSimulation()) {
	    ...
	    mField.getObject("bestTag")
		    .setPose(mVision.getTagLayout().getTagPose(bestTarget.getFiducialId()).toPose2d());
	}
    }
}
